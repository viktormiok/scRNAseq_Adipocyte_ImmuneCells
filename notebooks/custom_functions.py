...
def cell_percent(adata, cluster, condition, color, norm='index', xlabel='cell cluster', ylabel='cell count',
    ...
    tmp.plot.bar(stacked=True, color=color).legend(loc=leg_loc)
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    if save is not None:
        plt.savefig(save+title+'.pdf', bbox_inches='tight')
    if table is True:
        if save is not None:
